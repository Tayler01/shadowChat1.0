...
Interactive mode renders a split console layout on wide terminals. The center pane is reserved for recent chat messages only, while status, admin output, protocol activity, and other live feed details stay in a side pane. The input row has a blinking cursor so the entry field remains visually obvious while incoming messages redraw. `/live` toggles near-realtime polling for the active group or DM thread without leaving the chat view.
