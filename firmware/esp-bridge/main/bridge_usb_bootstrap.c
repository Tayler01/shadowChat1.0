...
static const char *START_HERE_CMD =
    ...
    "echo.\r\n"
    "powershell -NoProfile -ExecutionPolicy Bypass -File \"%~dp0RECEIVE.PS1\" -Output \"%USERPROFILE%\\Desktop\"\r\n"
    "echo.\r\n"
    "pause\r\n";
...
static const char *RECEIVER_PS1 =
    ...
    "  try{ $sp.Open(); Start-Sleep -Milliseconds 900; $sp.DiscardInBuffer(); $sp.WriteLine('bootstrap ping'); $until=(Get-Date).AddSeconds(3); while((Get-Date) -lt $until){ try{ if($sp.ReadLine() -match 'SHADOWCHAT_BRIDGE_READY'){ return $sp } }catch [TimeoutException]{} } }catch{} \r\n"
    ...
    "}\r\n"
    "$sp=$null; $fs=$null; $path=''; $sha=''; $seq=0; $bytes=0\r\n"
    "try{\r\n"
    "  New-Item -ItemType Directory -Force -Path $Output | Out-Null\r\n"
    "  $sp=Open-BridgePort $Port\r\n"
    "  $sp.DiscardInBuffer(); $sp.WriteLine('bundle get windows_bundle')\r\n"
    ...
    "      $path=Join-Path $Output $name; $sha=[string]$frame.sha256; $seq=0; $bytes=0\r\n"
    "      $fs=[IO.File]::Open($path,[IO.FileMode]::Create,[IO.FileAccess]::Write)\r\n"
    "      Write-Host \"Receiving $name to $path\"\r\n"
    ...
    "      Write-Host \"Transfer complete: $path\"; Write-Host \"SHA256: $actual\"; break\r\n"
    "    }\r\n"
    "  }\r\n"
