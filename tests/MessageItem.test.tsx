...
test('keeps quick reactions over the message when the iPhone keyboard lifts the composer', async () => {
  const footer = document.createElement('div')
  footer.setAttribute('data-mobile-chat-footer', 'true')
  document.body.appendChild(footer)
  const rectSpy = jest.spyOn(HTMLElement.prototype, 'getBoundingClientRect')
  ...
  Object.defineProperty(window, 'visualViewport', {
    configurable: true,
    value: { offsetTop: 0, offsetLeft: 0, width: 390, height: 520 },
  })
  ...
  try {
    render(
      <MessageItem
        message={{ ...baseMessage, message_type: 'text', content: 'keyboard quick reaction' } as Message}
        onEdit={async () => {}}
        onDelete={async () => {}}
        onTogglePin={async () => {}}
        onToggleReaction={async () => {}}
    ...
    await waitFor(() => {
      const rail = screen.getByRole('toolbar', { name: /quick reactions/i })
      expect(Number.parseFloat(rail.style.top)).toBeGreaterThan(440)
      expect(Number.parseFloat(rail.style.top)).toBeLessThan(470)
    })
  } finally {
    footer.remove()
    rectSpy.mockRestore()
