...
export function HypeCelebrationController() {
  ...
  const presentation = getHypeCelebrationPresentation({
    mode,
    intensity,
    prefersReducedMotion,
  })
  const displayDurationMs = getHypeDisplayDurationMs({
    mode,
    intensity,
    prefersReducedMotion,
    isPhone,
  ...
  const particles = useMemo(() => {
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      return {
        ...
        duration: `${Math.max(1_400, displayDurationMs - 800 + (index % 6) * 90)}ms`,
  ...
  }, [displayDurationMs, intensity, presentation])
  ...
  return createPortal(
    <div
      ...
      className={`hype-celebration hype-celebration--${presentation}`}
      data-hype-intensity={intensity}
      data-hype-presentation={presentation}
      data-reduced-motion={prefersReducedMotion ? 'true' : 'false'}
      style={{
        '--hype-intensity': intensity,
        '--hype-display-duration': `${displayDurationMs}ms`,
    ...
    >
      {presentation === 'fullscreen' ? (
        ...
        <div className="hype-celebration__compact-card">
          <Sparkles className="hype-celebration__compact-icon" aria-hidden="true" />
          <div className="hype-celebration__copy hype-celebration__copy--compact">
            <p className="hype-celebration__eyebrow">Hype x{activeCelebration.intensity}</p>
            <h2 className="hype-celebration__title">{title}</h2>
            {subtitle && <p className="hype-celebration__subtitle">{subtitle}</p>}
          </div>
          <button
            type="button"
            className="hype-celebration__dismiss hype-celebration__dismiss--compact"
            onClick={dismissCelebration}
            aria-label="Dismiss Hype celebration"
          >
            <X className="h-4 w-4" />
