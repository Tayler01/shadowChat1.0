  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[var(--bg-overlay)] px-4 backdrop-blur-md">
      <div
        ...
        className="popup-surface max-h-[calc(100dvh-2rem)] w-full max-w-xl overflow-y-auto rounded-[var(--radius-lg)] p-6"
      >
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <h2 id="notification-setup-title" className="text-xl font-semibold text-[var(--text-primary)]">
              {guidance.title}
            </h2>
        ...
        <div className="mt-5 flex flex-col gap-3 sm:flex-row">
          {guidance.canRequestNow && (
            <Button onClick={() => void onEnable()} loading={saving} className="w-full sm:flex-1">
          ...
          {guidance.requiresInstall && canInstall && (
            <Button onClick={() => void onInstall()} variant="secondary" loading={saving} className="w-full sm:flex-1">
              <Smartphone className="mr-2 h-4 w-4" />
              Install App
            </Button>
          )}

          <Button onClick={() => void handleCopy()} variant="ghost" className="w-full sm:flex-1">
        ...
        <p className="mt-4 flex items-start gap-2 text-xs text-[var(--text-muted)]">
          ...
          Follow the steps, then return here so Shadow Chat can recheck notification permission.
        </p>
      </div>
    </div>
  )
