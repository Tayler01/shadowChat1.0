...
export function ActiveUsersButton({ resetStatus, variant = 'compact' }: ActiveUsersButtonProps) {
        ...
        {variant === 'nav' ? (
          <>
            <span className="relative mb-1 flex h-8 w-8 items-center justify-center rounded-full bg-[var(--nav-icon-bg)]">
              ...
              <span className="theme-unread-badge absolute -right-1 -top-1 rounded-full px-1 text-[0.625rem] leading-none" aria-hidden="true">{activeUsers.length > 99 ? '99+' : activeUsers.length}</span>
            </span>
            <span>Active</span>
          </>
        ) : (
