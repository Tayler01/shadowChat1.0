          ...
          <div className="mx-auto max-w-3xl">
            ...
            {state.step === 'details' && (
              <section className="space-y-5" aria-labelledby="creator-details-title">
                ...
                <label className="block space-y-2"><span className="text-sm font-medium">Tags <span className="font-normal text-[var(--text-muted)]">(optional)</span></span><input value={tagsText} onChange={event => { setTagsText(event.target.value); dispatch({ type: 'set-value', key: 'tags', value: event.target.value.split(',') }) }} className="obsidian-input min-h-12 w-full rounded-[var(--radius-md)] px-3 text-base" placeholder="folklore, travel, behind-the-scenes" /><span className="text-xs text-[var(--text-muted)]">Add up to 8 comma-separated tags, or leave this blank.</span></label>
              </section>
            )}
