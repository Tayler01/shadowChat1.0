...
export function ShadoLiveStage({
  ...
  return (
    <div className="flex h-[var(--shadowchat-app-height,var(--shadowchat-visual-viewport-height,100dvh))] min-h-0 flex-1 flex-col overflow-hidden bg-[#050505] pb-[var(--shadowchat-mobile-scroll-keyboard-inset,0px)]" data-testid="shado-live-real-stage">
      ...
      <div className="shado-live-stage-grid grid min-h-0 flex-1 grid-rows-[minmax(8rem,0.85fr)_minmax(13rem,1.15fr)] lg:grid-cols-[minmax(0,1fr)_23rem] lg:grid-rows-1">
        <main className="shado-live-stage-visual grid min-h-0 grid-rows-[minmax(0,1fr)_auto] gap-2 overflow-hidden p-3 sm:p-4 lg:p-5">
          <section className="relative min-h-0 overflow-hidden rounded-[1.75rem] border border-[#d7aa46]/28 bg-[radial-gradient(circle_at_50%_20%,rgba(215,170,70,0.13),transparent_42%),#0b0906] shadow-[inset_0_1px_rgba(255,255,255,0.05)]" data-testid="shado-live-real-stage-visual">
            <div className="relative flex h-full min-h-[8rem] flex-col items-center justify-center px-4 py-3 text-center max-[360px]:hidden">
              {activeSpeaker ? (
                <>
                  <span className={`grid h-16 w-16 shrink-0 place-items-center rounded-full border-2 bg-[#171008]/90 text-xl font-black text-[#f6dda1] sm:h-20 sm:w-20 sm:text-2xl ${mediaByIdentity.get(activeSpeaker.providerIdentity)?.speaking ? 'border-[#f0d381] shadow-[0_0_0_8px_rgba(232,189,88,0.09),0_18px_48px_rgba(0,0,0,0.5)]' : 'border-[#8b6a32]'}`} aria-hidden="true">
                    {initials(activeSpeaker.displayName)}
                  </span>
                  <p className="mt-2 text-lg font-bold text-white">{activeSpeaker.displayName}</p>
              ...
              <div className="mt-3 flex -space-x-1.5" role="list" aria-label="People on stage">
                {stageParticipants.map(participant => {
                  ...
                  return (
                    <span key={participant.userId} role="listitem" aria-label={`${participant.displayName}, ${roleLabel(participant.role)}${speaking ? ', speaking' : ''}`} className={`grid h-10 w-10 place-items-center rounded-full border bg-black text-xs font-bold text-white ${speaking ? 'border-[#f0d381]' : 'border-[#6d5730]'}`}>
            ...
            <div className="relative hidden h-full min-h-0 items-center gap-3 px-4 max-[360px]:flex">
              <span className="grid h-12 w-12 shrink-0 place-items-center rounded-full border-2 border-[#e8bd58] bg-[#171008]/90 text-sm font-black text-[#f6dda1]">{activeSpeaker ? initials(activeSpeaker.displayName) : 'SL'}</span>
        ...
        <aside className="flex min-h-0 flex-col overflow-hidden border-t border-white/10 bg-[#0b0a09] shadow-[0_-16px_42px_rgba(0,0,0,0.34)] lg:border-l lg:border-t-0 lg:shadow-none" data-testid="shado-live-real-panel">
          ...
          <div ref={panelScrollRef} id={`shado-live-real-panel-${panel}`} role="tabpanel" aria-labelledby={`shado-live-real-tab-${panel}`} tabIndex={0} className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#d7aa46]">
            {panel === 'chat' && (
              <div className="space-y-4" data-testid="shado-live-real-chat-panel">
                ...
                {room.messages.map(message => (
                  <div key={message.id} className="flex gap-3">
                    <span className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-full border border-[#6d5730] bg-black text-xs font-bold text-white" aria-hidden="true">{initials(message.senderDisplayName)}</span>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="min-w-0 flex-1 truncate text-xs font-semibold text-[#e8bd58]">{message.senderDisplayName}</p>
                  ...
                  return (
                    <div key={participant.userId} className="rounded-2xl border border-white/10 bg-white/[0.025] p-3">
                      <div className="flex items-center gap-3">
                        <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-full border bg-black text-xs font-bold text-white ${live?.speaking ? 'border-[#f0d381]' : 'border-[#6d5730]'}`} aria-hidden="true">{initials(participant.displayName)}</span>
          ...
          {panel === 'chat' && (
            <form onSubmit={submitMessage} className="flex shrink-0 items-end gap-2 border-t border-white/10 bg-[#0b0a09] p-3" data-testid="shado-live-real-composer">
              <label className="min-w-0 flex-1"><span className="sr-only">Message the live room</span><textarea ref={composerRef} value={draft} onChange={event => setDraft(event.target.value)} disabled={!controller.controlsEnabled} maxLength={500} rows={1} placeholder={controller.controlsEnabled ? 'Message the room' : 'Chat locked while room state syncs'} className="max-h-24 min-h-12 w-full resize-none rounded-2xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none placeholder:text-[#777168] focus:border-[#d7aa46]/60 focus:ring-2 focus:ring-[#d7aa46]/20 disabled:opacity-55" /></label>
              <button type="submit" disabled={!controller.controlsEnabled || !draft.trim() || controller.commandBusy === 'send_message'} aria-label="Send live room message" className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[#d7aa46] text-black focus:outline-none focus:ring-2 focus:ring-[#f4d985] focus:ring-offset-2 focus:ring-offset-black disabled:opacity-35">{controller.commandBusy === 'send_message' ? <Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" /> : <Send className="h-5 w-5" aria-hidden="true" />}</button>
            </form>
          )}
        </aside>
      </div>
