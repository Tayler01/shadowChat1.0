...
export function ShadowWarCardView({
  ...
  if (hidden || !card) {
    return (
      <div className={cn(
        'relative flex aspect-[3/4] min-h-0 flex-col items-center justify-center overflow-hidden rounded-[var(--radius-md)] border border-[rgba(215,170,70,0.22)] bg-[radial-gradient(circle_at_50%_8%,rgba(215,170,70,0.2),rgba(18,19,21,0.96)_52%,rgba(6,7,8,0.98))] text-[var(--theme-accent-readable)] shadow-[0_14px_32px_rgba(0,0,0,0.36)]',
        compact ? 'w-16' : 'w-full'
      )}>
        <img
  ...
  return (
    <button
      ...
      disabled={!onClick}
      className={cn(
        'group relative aspect-[3/4] min-h-0 overflow-hidden rounded-[var(--radius-md)] border text-left shadow-[0_14px_32px_rgba(0,0,0,0.36)] transition-[border-color,box-shadow,transform] duration-[var(--dur-med)]',
        selected
