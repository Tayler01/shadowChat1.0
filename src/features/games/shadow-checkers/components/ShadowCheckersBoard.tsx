...
export function ShadowCheckersBoard({
  ...
  const boardGrid = (
    <div
      className={cn(
        'grid h-full w-full grid-cols-8 grid-rows-8 overflow-visible',
        isCinematic ? 'rounded-[0.15rem]' : 'overflow-hidden rounded-[0.9rem] border border-black/70'
      )}
    >
      {squares.map(square => {
        ...
        return (
          <button
            ...
            type="button"
            data-checkers-square={`${square.row}-${square.col}`}
            data-checkers-piece-id={piece?.id}
            data-checkers-playable={square.playable ? 'true' : 'false'}
  ...
  return (
    <div className="relative mx-auto w-full max-w-[min(94vw,42rem)] [perspective:1100px]">
      ...
      <div
        className={cn(
          'relative aspect-square rounded-[1.25rem] border shadow-[0_34px_80px_rgba(0,0,0,0.72),inset_0_0_0_1px_rgba(255,239,184,0.08)]',
          isCinematic
            ? 'overflow-hidden border-[#f0d381]/55 bg-cover bg-center'
            : 'border-[#d7aa46]/45 bg-[#050505] p-[3.2%] [transform:rotateX(10deg)]'
        )}
        style={isCinematic ? { backgroundImage: `url(${SHADOW_CHECKERS_ASSETS.boardCinematic})` } : undefined}
      >
        {isCinematic ? (
          <div className="absolute" style={CINEMATIC_BOARD_GRID_FRAME}>
            {boardGrid}
          </div>
        ) : boardGrid}
      </div>
