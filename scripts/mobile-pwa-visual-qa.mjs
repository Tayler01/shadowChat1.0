...
async function runProfile(profile, accountA, accountB) {
  ...
  try {
    ...
    await auditPage(page, profile, '07-dm-list')
    ...
    await auditPage(page, profile, '10-dm-list-after-back')
    ...
    await auditPage(page, profile, '18-account-profile')
...
async function auditPage(page, profile, flow, options = {}) {
  ...
  const metrics = await page.evaluate(({ expectComposer }) => {
  ...
  }, { expectComposer: Boolean(options.composer) })
