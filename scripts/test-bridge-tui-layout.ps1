...
Assert-True ((Get-VersionLabel) -match "v0\.1\.31-kali-grid 2026-04-28") "Header version label should expose the running TUI bundle version."
Assert-True (((Get-SidebarLines) -join "`n") -match "tools 0\.1\.31-kali-grid") "Sidebar should expose the running TUI bundle version."
Assert-True ((Get-WorkspaceBar) -match "\[1:g\*\].*\[2:dm\].*\[3:adm\]") "Workspace bar should expose the active i3-style group tile."
Assert-True ((Get-TopBarLine 120).Length -eq 120) "Top status bar should fit the terminal width exactly."
Assert-True ((Get-PaneHeaderLine 120 $false $true 0 87 30).Length -eq 120) "Two-pane header should fit the terminal width exactly."
