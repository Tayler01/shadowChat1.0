/*
  # Realtime Chat Platform Database Schema - Safe Migration
...
-- Add missing columns to dm_conversations table if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'dm_conversations' AND column_name = 'updated_at') THEN
    ALTER TABLE dm_conversations ADD COLUMN updated_at timestamptz DEFAULT now();
  END IF;
END $$;
...
-- Add missing columns to dm_messages table if they don't exist
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'dm_messages' AND column_name = 'message_type') THEN
    ALTER TABLE dm_messages ADD COLUMN message_type text DEFAULT 'text';
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'dm_messages' AND column_name = 'read_by') THEN
    ALTER TABLE dm_messages ADD COLUMN read_by uuid[];
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'dm_messages' AND column_name = 'updated_at') THEN
    ALTER TABLE dm_messages ADD COLUMN updated_at timestamptz DEFAULT now();
  END IF;
END $$;
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'message_reactions' AND table_schema = 'public') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_presence' AND table_schema = 'public') THEN
...
-- Drop existing policies to avoid conflicts (safe operation)
DO $$
BEGIN
  -- Users policies
  ...
  DROP POLICY IF EXISTS "Users can update own conversations" ON dm_conversations;
  ...
  DROP POLICY IF EXISTS "Users can read all reactions" ON message_reactions;
...
CREATE POLICY "Users can update own conversations" ON dm_conversations FOR UPDATE TO authenticated 
  USING (auth.uid() = ANY(participants));
...
CREATE POLICY "Users can delete own DM messages" ON dm_messages FOR DELETE TO authenticated 
  USING (auth.uid() = sender_id);

-- Create RLS Policies for message reactions
CREATE POLICY "Users can read all reactions" ON message_reactions FOR SELECT TO authenticated USING (true);
CREATE POLICY "Users can add own reactions" ON message_reactions FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can read all presence" ON user_presence FOR SELECT TO authenticated USING (true);
CREATE POLICY "Users can update own presence" ON user_presence FOR ALL TO authenticated 
