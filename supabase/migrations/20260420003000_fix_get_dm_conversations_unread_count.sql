/*
  # Fix DM conversations RPC unread_count type

  The function signature declares `unread_count integer`, but `count(*)`
  returns `bigint`. Supabase RPC rejects the result with a 400 when the
  query shape no longer matches the declared return type.
*/
